...
const normalizeText = (text) => {
    ...
    // Remove JSON fragments like {"title": "..."} that might leak as AI artifacts
    const jsonMetadataRegex = /\{[^{}]*?"[^{}]*?":\s*[^{}]*?\}/g;
    
    return text
        .replace(jsonMetadataRegex, "")
        .replace(/\r\n/g, "\n")
        .replace(/\r/g, "\n")
        .replace(/[ \t]+/g, " ")
        .replace(/\n{3,}/g, "\n\n")
...
app.post("/export/docx", async (req, res) => {
    ...
    try {
        ...
        const doc = new Document({
            title: cleanBookTitle,
            creator: cleanAuthor,
            description: docxDesc,
            subject: "Generated by W4U Writing Wizard",
            sections: [{
                properties: {
                    page: {
                        margin: {
